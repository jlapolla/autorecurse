...
class ConditionalChunkParser(Iterator[Iterator[T]]):

    @staticmethod
    def make(iterator: Iterator[T], condition: StreamCondition[T]) -> 'ConditionalChunkParser':
        """
        Specification domain:

        - iterator.is_at_start
        """
        instance = ConditionalChunkParser()
        ConditionalChunkParser._setup(instance, iterator, condition)
        return instance

    @staticmethod
    def _setup(instance: 'ConditionalChunkParser', iterator: Iterator[T], condition: StreamCondition[T]) -> None:
        instance._iterator = iterator
        instance._condition = condition
        instance._chunk = None
        instance._is_at_end = False
        pass

    @property
    def current_item(self) -> Iterator[T]:
        return self._chunk
    ...
    def move_to_next(self) -> None:
        # assert self.is_at_start <-> self._iterator.is_at_start
        if self._iterator.is_at_start:
            # assert self.is_at_start
            self._move_to_next_start_start()
        elif self._iterator.has_current_item:
            # assert self.has_current_item
            self._move_to_next_intermediate_intermediate()
        else:
            # assert self._iterator.is_at_end
            # assert self.has_current_item
            self._chunk = None
            self._is_at_end = True

    def _move_to_next_start_start(self) -> None:
        """
        Specification domain:

        - self.is_at_start
        - self._iterator.is_at_start
        """
        self._parse_next_chunk([], False)
        # assert self.has_current_item \/ self.is_at_end

    def _move_to_next_intermediate_intermediate(self) -> None:
        """
        Specification domain:

        - self.has_current_item
        - self._iterator.has_current_item
        """
        list_ = []
        chunk_started = False
        self._condition.current_item = self._iterator.current_item
        if self._condition.condition:
            list_.append(self._iterator.current_item)
            chunk_started = True
        self._parse_next_chunk(list_, chunk_started)
        # assert self.has_current_item \/ self.is_at_end

    def _parse_next_chunk(self, list_: typing.List[T], chunk_started: bool) -> None:
        self._iterator.move_to_next()
        chunk_ended = False
        while not (chunk_ended or self._iterator.is_at_end):
            self._condition.current_item = self._iterator.current_item
            if self._condition.condition:
                list_.append(self._iterator.current_item)
                self._iterator.move_to_next()
                ...
                if chunk_started:
                    ...
                    self._iterator.move_to_next()
        # assert ~(chunk_ended /\ self._iterator.is_at_end)
        # assert chunk_ended \/ self._iterator.is_at_end
        # assert chunk_ended -> chunk_started
        if chunk_started:
            self._chunk = ListIterator.make(list_)
        ...
        # assert ~(chunk_ended /\ self._iterator.is_at_end)
        # assert chunk_ended \/ self._iterator.is_at_end
        # assert self._iterator.has_current_item \/ self._iterator.is_at_end
        # assert self.is_at_end -> self._iterator.is_at_end

    def move_to_end(self) -> None:
        self._iterator.move_to_end()
        self._chunk = None
        self._is_at_end = True
